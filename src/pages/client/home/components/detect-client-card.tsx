...
export default function DetectClientCard({
  clientes,
  currentDetected,
  selected,
  onSelect,
  detectedClient,
}: DetectClientCardProps) {
  // ============ APP MODE ===========
    ...
    if (navigator.permissions) {
      navigator.permissions.query({ name: 'geolocation' as PermissionName }).then((result) => {
        if (result.state === 'denied') {
          alert(
            'Atenção: O acesso à localização está bloqueado. O aplicativo não funcionará corretamente.'
          )
        }
  ...
  const getLocationTitle = () => {
    ...
    if (clientesProximos.length === 1)
      return `[${clientesProximos[0].codigo}] ${clientesProximos[0].nome_fantasia}`
  ...
  const getLocationDescription = () => {
    ...
    if (clientesProximos.length > 0)
      return `Encontramos ${clientesProximos.length} cliente(s) próximo(s) de você.`
  ...
  return (
    ...
    >
      <CardHeader className='p-2 shrink-0 '>
        <div
          className={cn('size-14 flex items-center justify-center rounded-full', {
